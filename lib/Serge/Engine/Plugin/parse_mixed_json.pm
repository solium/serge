package Serge::Engine::Plugin::parse_mixed_json;
...
sub name {
    return 'Mixed Chrome extension messages.json parser plugin when outputing key value';
}
...
sub parse {
    ...
    my $output_messages = \%$messages;

    if ($self->{import_mode} or $lang) {
        $output_messages = {};
    }
    ...
    foreach my $key (sort keys %$messages) {
        ...
        if ($self->{import_mode}) {
            $output_messages->{$key} = &$callbackref($msg, $key, $key, undef, $lang, $key);
        } else {
            my $message = &$callbackref($msg->{message}, $key, $msg->{description}, undef, $lang, $key);

            if ($lang and $message) {
                $output_messages->{$key} = $message;
            }
        }
    }
    ...
    return to_json($output_messages, {pretty => 1, indent_length => 3, canonical => 1, escape_slash => 1});
}
